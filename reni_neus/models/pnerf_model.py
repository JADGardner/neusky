...
@dataclass
class PNeRFModelConfig(NeuSModelConfig):
    """NeusFacto Model Config"""

    _target: Type = field(default_factory=lambda: PNeRFModel)
...
class PNeRFModel(NeuSModel):
    ...
    def populate_modules(self):
        ...
        self.renderer_rgb = RGBRenderer(background_color=torch.Tensor([0.0, 0.0, 0.0]))
    ...
    def get_image_metrics_and_images(
        ...
        if "fg_mask" in batch:
            fg_mask = batch["fg_mask"].to(self.device)  # H, W, 1
            accu = outputs["accumulation"]  # H, W, 1
            combined_accumulation = torch.cat([accu, fg_mask], dim=1)  # H, W*2, 1
            images_dict["combined_accumulation"] = combined_accumulation
