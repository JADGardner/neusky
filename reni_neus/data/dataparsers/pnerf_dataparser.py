...
@dataclass
class PNeRFDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: PNeRFDataParser)
...
class PNeRFDataParser(DataParser):
    ...
    config: PNeRFDataParserConfig

    def __init__(self, config: PNeRFDataParserConfig):
    ...
    def _generate_dataparser_outputs(self, split: str = "train"):
        ...
        # set the average z position to be 0 where position is the last column
        poses[:, 2, 3] -= torch.mean(poses[:, 2, 3], dim=0)
