...
def evaluate_synthetic(scene: str, device: str = None, method: str = "neusky"):
    ...
    if estimated_path.exists():
        ...
        mapping = {
            ...
            "estimated_lpips": "estimation/lpips_masked",
        }
        for out_key, metric_key in mapping.items():
            value = _aggregate(metrics, metric_key)
            if value is not None:
...
def write_synthetic_per_scene_table(results, output: Path, methods):
    """Per-scene companion to write_synthetic_table: one block per scene,
    one row per method, best value per scene/metric in bold."""
    decimals = {
        "known_psnr": 2, "known_ssim": 3, "known_lpips": 3,
        "estimated_psnr": 2, "estimated_ssim": 3, "estimated_lpips": 3,
        "albedo_psnr": 2, "normal_mae": 2, "depth_rmse": 2,
    ...
    lines = [
        ...
        "Scene & Method & PSNR\\textsubscript{EA} $\\uparrow$ & SSIM $\\uparrow$ & "
        "LPIPS $\\downarrow$ & PSNR $\\uparrow$ & SSIM $\\uparrow$ & LPIPS $\\downarrow$ & "
        "Albedo PSNR $\\uparrow$ & Normal MAE $\\downarrow$ & Depth RMSE $\\downarrow$" + row_end,
        "\\midrule",
    ]
    for si, (scene, scene_label) in enumerate(SYNTHETIC_SCENES.items()):
        scene_rows = {m: results.get(f"synthetic/{m}/{scene}") for m in methods}
        best = {}
        for metric in SYNTHETIC_METRICS:
            vals = [r[metric] for r in scene_rows.values() if r and metric in r]
            if vals:
                best[metric] = min(vals) if metric in lower_better else max(vals)
        for mi, method in enumerate(methods):
            label = SYNTHETIC_METHODS.get(method, method)
            r = scene_rows.get(method)
            first = f"\\multirow{{{len(methods)}}}{{*}}{{{scene_label}}}" if mi == 0 else ""
            if not r:
                lines.append(f"{first} & {label} & "
                             + " & ".join(["---"] * len(SYNTHETIC_METRICS)) + row_end)
                continue
            cells = []
            for metric in SYNTHETIC_METRICS:
                if metric not in r:
                    ...
                    continue
                value = r[metric]
                cell = f"{value:.{decimals[metric]}f}"
                if metric in best:
                    is_best = (value <= best[metric] if metric in lower_better
                               else value >= best[metric])
                    if is_best:
                        cell = f"\\textbf{{{cell}}}"
                cells.append(cell)
            lines.append(f"{first} & {label} & " + " & ".join(cells) + row_end)
        if si < len(SYNTHETIC_SCENES) - 1:
            lines.append("\\midrule")
    lines += ["\\bottomrule", "\\end{tabular}"]
    output.parent.mkdir(parents=True, exist_ok=True)
    output.with_suffix(".tex").write_text("\n".join(lines) + "\n")
    print(f"[saved] {output}.tex")
...
def main():
    ...
    if "synthetic" in args.tables:
        ...
        write_synthetic_per_scene_table(
            results, args.output_dir / "synthetic_per_scene", methods)
