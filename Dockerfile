...
RUN python3.10 -m pip install omegaconf torchtyping

RUN git clone https://github.com/JADGardner/nerfstudio.git && \
    cd nerfstudio && \
    python3.10 -m pip install -e .

RUN git clone https://github.com/JADGardner/ns_reni.git && \
    cd ns_reni && \
    python3.10 -m pip install -e . && \
    cd ..
