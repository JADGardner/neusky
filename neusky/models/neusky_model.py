...
from neusky.model_components.renderers import (
...
from neusky.data.datamanagers.neusky_datamanager import NeuSkyDataManager
...
@dataclass
class NeuSkyFactoModelConfig(NeuSFactoModelConfig):
...
class NeuSkyFactoModel(NeuSFactoModel):
    ...
    def get_outputs(
        ...
        if self.training:
            ...
            albedo = self.albedo_renderer(rgb=field_outputs[NeuSkyFieldHeadNames.ALBEDO], weights=weights)
        else:
        ...
        if self.show_ddf_static_flag:
            vis_outputs = self.visibility_field.get_outputs_for_camera_ray_bundle(
                ray_bundle, neusky=None, show_progress=False
            )
    ...
    def get_image_metrics_and_images(
        ...
        mse = torch.mean((image - rgb) ** 2)

        # all of these metrics will be logged as scalars
        metrics_dict = {"psnr": float(psnr.item()), "ssim": float(ssim)}  # type: ignore
        metrics_dict["lpips"] = float(lpips)
        metrics_dict["mse"] = float(mse)
