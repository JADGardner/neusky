...
from neusky.utils.utils import random_points_on_unit_sphere, ray_sphere_intersection
...
class DDFModel(Model):
    ...
    def get_outputs(self, ray_bundle: RayBundle, batch, neusky, stop_gradients: bool = True):
        if self.field is None:
        ...
        expected_termination_dist = field_outputs[NeuSkyFieldHeadNames.TERMINATION_DISTANCE]
        outputs["expected_termination_dist"] = expected_termination_dist

        if NeuSkyFieldHeadNames.PROBABILITY_OF_HIT in field_outputs:
            outputs["expected_probability_of_hit"] = field_outputs[NeuSkyFieldHeadNames.PROBABILITY_OF_HIT]
        ...
        if (self.config.loss_inclusions["sdf_l1_loss"] or self.config.loss_inclusions["sdf_l2_loss"]) and self.training:
            if neusky is not None:
                termination_points = positions + directions * expected_termination_dist.unsqueeze(-1)
                if stop_gradients:
                    with torch.no_grad():
                        sdf_at_termination = neusky.field.get_sdf_at_pos(termination_points)
        ...
        if self.config.loss_inclusions["multi_view_loss"] and self.training and batch is not None:
            ...
            outputs["multi_view_expected_termination_dist"] = field_outputs[NeuSkyFieldHeadNames.TERMINATION_DISTANCE]
        ...
        if self.config.loss_inclusions["sky_ray_loss"] and self.training and batch is not None:
            ...
            outputs["sky_ray_expected_termination_dist"] = field_outputs[NeuSkyFieldHeadNames.TERMINATION_DISTANCE]
    ...
    def forward(self, ray_bundle: RayBundle, batch, neusky, stop_gradients: bool = True) -> Dict[str, torch.Tensor]:
        ...
        return self.get_outputs(ray_bundle, batch, neusky, stop_gradients=stop_gradients)
    ...
    def get_outputs_for_camera_ray_bundle(
        self, camera_ray_bundle: RayBundle, neusky=None, show_progress=False
                ...
                for i in range(0, num_rays, num_rays_per_chunk):
                    ...
                    if self.config.compute_normals:
                        with torch.enable_grad():
                            outputs = self.forward(ray_bundle=ray_bundle, batch=None, neusky=neusky)
                    else:
                        outputs = self.forward(ray_bundle=ray_bundle, batch=None, neusky=neusky)
                    # move to cpu
    ...
    def get_image_dict(
        ...
        if NeuSkyFieldHeadNames.PROBABILITY_OF_HIT in outputs:
            expected_probability_of_hit = outputs[NeuSkyFieldHeadNames.PROBABILITY_OF_HIT]
